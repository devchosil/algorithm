...
//전체 케이스 숫자만큼 반복
for(i=1; i<=T; i++) {
    let num = +input[i].trim().split(' ')[0]; //케이스의 첫번째 요소
    let text = input[i].trim().split(' ')[1]; //케이스의 두번째 요소
    let result = '';

    for(j=0; j<text.length; j++) { //문자의 길이!!(계속 num으로 해서 에러났음)
        result += text[j].repeat(num);  //str.repeat(count)
    }
    console.log(result);
}