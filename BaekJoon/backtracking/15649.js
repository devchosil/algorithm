...
// let visited = new Array(N).fill(false); // 방문 여부 + 순서까지 알 수 있음
...
//     // console.log("depth, selected",depth, selected);
...
let arr = [];
for(let i=1;i<=N;i++) arr.push(i); // 대상 array
let visited = new Array(N).fill(false); // 방문한 array status
let selected = []; // 선택된 array의 index
let answer = '';

function dfs(arr, depth) {
  if(depth === M) {
    // for(i of selected) answer += arr[i] + '\n'; 
    let result = [];
    for(let i of selected) result.push(arr[i]);
